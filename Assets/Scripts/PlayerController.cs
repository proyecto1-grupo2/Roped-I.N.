...
//Nos aseguramos de que sea dinámico  
            ...
            /*//para parar al jugador cuando deje de moverse y no se deslice
            ...
            //Salto cuando pulso el boton de salto y si estoy en el suelo
            ...
            GameManager.instance.direccion(dirGancho);//pasa la direccion al GameManager
        }
        else if (enganchado )  //Estado Enganchado
        {

           // GetComponent<Rigidbody2D>().isKinematic = true;//lo cambiamos a kinematic para moverlo con transform
    ...
    private void FixedUpdate()//Mueve al personaje
    {
        if (enganchado == false) //aseguramos que no se ejecute cuando sea cinematico
        {
            ...
            if (rb != null && Mathf.Abs(rb.velocity.x) < maxSpeed ) //Limitamos la velocidad del jugador con el rb.velocity.x
            {
                ...
                */
                rb.AddForce(movement * acceleration); //Movimiento horizontal
                
                //Debug.Log("la velocity"+rb.velocity);
            }
            if(rb.velocity.y < -10)//para que no caiga muy rapido
            {
                rb.velocity = new Vector2(rb.velocity.x, -10);
            }
            /*else if (rb != null && moveX==0) //Limitamos la velocidad del jugador con el rb.velocity.x
            {
                Debug.Log("HOLA");
            ...
            if (jump)
            {
                rb.AddForce(Vector2.up * jumpForce, ForceMode2D.Impulse);   //Salto
                jump = false;                                               //La variable jump se hace falsa despues de hacer el salto para que no se ejecute el salto mas veces en el FixedUpdate
            }
        }
    }
